 ...
 * @param {Boolean} isVerified Whether to activate or deactivate these accounts. Defaults to `true` - 'activate'
 */
const updatePropertyResidentVerification = async (
  ...
  return updateResident({ isVerified }, { propertyId }, true).then(() => {
    ...
    residentsToUpdate.forEach(async (resident) => {
      ...
      await UserService.updateUser({ isVerified }, userUpdateFilter, true);
    });
  });
};
